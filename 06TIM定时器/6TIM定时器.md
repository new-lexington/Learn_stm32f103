...
> 2. 在配置结构体成员时，需要给不需要使用的结构体成员设一个初始值，否则结构体成员的值是不确定的，这会导致一些问题。可以在配置结构体时给每个成员都配置一下，或者使用XX_XXStructInit()函数给结构体赋一个初始值，再修改几个成员的值。
> 3. 输出比较电路的OC1输出怎么映射到GPIO引脚上呢？其实在引脚定义表上就已经固定好了，OC1输出固定是PA0。但是配置GPIO时注意配置成“复用推挽输出”模式，因为GPIO输出框图中显示，只有复用输出模式才能使信号来自片上外设。
> 4. 引脚重映射。进行引脚重映射时，需要不断对照参考手册及引脚定义表。若对 <u>调试端口</u> 进行引脚重映射，需要三步：开启AFIO时钟，进行引脚重映射，解除调试端口复用。若想对 <u>定时器和其他外设的复用引脚</u> 进行重映射，那就只需要前两步即可。
...
> 2. 测周法：两个上升沿内，以标准频率 $f_c$ 计次，得到N ，则频率 $f_x = f_c / N$。本质上就是直接测量一个周期的时间，适合测量低频信号，数据更新速度取决于待测信号频率，更新速度相对更快，同时因为只测量一个周期的时间，所以受噪声影响较大。
...
编程感想：
> 1. PWMI模式的配置。可以直接复制通道1的代码改进，但ST公司以为我们准备好了封装好了的函数: TIM_PWMIConfig，该函数能够自动完成通道2的配置，不需要再单独对通道2进行配置。
> 2. 测频率的性能。
> > 测频率的范围。测量下限：目前代码中的标准频率是1MHz，计数器最大只能记到65536，所以可测量的最低频率是1M / 65535，约为15Hz，如果信号频率再低，计数器就会发生溢出，如果想要降低最低频率，需要增大预分频器的值，降低标准频率。测量上限：最大频率并没有明确的界限，因为随着待测频率的增大，误差也会增大，如不考虑误差，则最大频率为1MHz，超过标准频率则无法测量，但信号频率接近1MHz时，误差已经非常大了，没有意义，所以最大频率取决于对误差的要求；如果误差要求为1 / N，则最大频率 = fc / N，如果要提高测量上限，可以降低PSC，增大标准频率，如果频率过高，应使用测频法。
> > 晶振误差。芯片外部晶振可能不过精确，经过几百上千万次的误差累计起来也会造成影响，可后期进行滤波处理。
