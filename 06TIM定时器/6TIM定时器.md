...
> 2. 测频率的性能。
> > (1) 测频率的范围。测量下限：目前代码中的标准频率是1MHz，计数器最大只能记到65536，所以可测量的最低频率是1M / 65535，约为15Hz，如果信号频率再低，计数器就会发生溢出，如果想要降低最低频率，需要增大预分频器的值，降低标准频率。
> > (2) 测量上限：最大频率并没有明确的界限，因为随着待测频率的增大，误差也会增大，如不考虑误差，则最大频率为1MHz，超过标准频率则无法测量，但信号频率接近1MHz时，误差已经非常大了，没有意义，所以最大频率取决于对误差的要求；如果误差要求为1 / N，则最大频率 = fc / N，如果要提高测量上限，可以降低PSC，增大标准频率，如果频率过高，应使用测频法。
